...
class TC_VNF_STATE_START_001(TestCase):
    ...
    18. Validate that the VNF is terminated and that all resources have been released by the VIM
    """
    REQUIRED_APIS = ('mano', 'traffic')
    REQUIRED_ELEMENTS = ('vnfd_id',)
    TESTCASE_EVENTS = ('instantiate_vnf', 'stop_vnf', 'start_vnf', 'terminate_vnf')
    ...
    def run(self):
        ...
        if self.traffic.does_traffic_flow(delay_time=constants.TRAFFIC_DELAY_TIME):
            raise TestRunError('Traffic is flowing', err_details='Normal traffic flew after VNF was stopped')

...
class TC_VNF_STATE_TERM_003(TestCase):
    """
    TC_VNF_STATE_TERM_003 VNF terminate from state STARTED with normal traffic load
    ...
    def setup(self):
        LOG.info('Starting setup for TC_VNF_STATE_TERM_003')
        ...
        LOG.info('Finished setup for TC_VNF_STATE_TERM_003')

    def run(self):
        LOG.info('Starting TC_VNF_STATE_TERM_003')
        ...
        if self.traffic.does_traffic_flow():
            raise TestRunError('Traffic is flowing', err_details='Traffic still flew after the VNF was stopped')

        LOG.info('TC_VNF_STATE_TERM_003 execution completed successfully')

...
class TC_VNFC_SCALE_OUT_005__VNF_MANUAL(TestCase):
    """
    TC_VNFC_SCALE_OUT_005__VNF_MANUAL Removal of virtualized specialized hardware acceleration for VNFC scale-in with
    manual scaling event generated by VNF
    ...
    5. Trigger a resize of the VNF resources to use more specialized hardware by instructing the VNF to instruct the
    ...
    11. Trigger a resize of the VNF resources to use less specialized hardware by instructing the VNF to instruct the
    ...
    def setup(self):
        LOG.info('Starting setup for TC_VNFC_SCALE_OUT_005__VNF_MANUAL')
        ...
        LOG.info('Finished setup for TC_VNFC_SCALE_OUT_005__VNF_MANUAL')

        return True

    def run(self):
        LOG.info('Starting TC_VNFC_SCALE_OUT_005__VNF_MANUAL')
        ...
        if self.vnf_instance_id is None:
            LOG.error('TC_VNFC_SCALE_OUT_005__VNF_MANUAL execution failed')
        ...
        # --------------------------------------------------------------------------------------------------------------
        # 5. Trigger a resize of the VNF resources to use more specialized hardware by instructing the VNF to instruct
        #    the MANO to scale out the VNF
        # --------------------------------------------------------------------------------------------------------------
        LOG.info('Trigger a resize of the VNF resources to use more specialized hardware by instructing the VNF to '
        ...
        if len(vnf_info.instantiated_vnf_info.vnfc_resource_info) != self.tc_input['scaling']['default_instances'] + \
            ...
            LOG.error('TC_VNFC_SCALE_OUT_005__VNF_MANUAL execution failed')
            LOG.debug('VNF did not scale out')
            self.tc_result['overall_status'] = constants.TEST_FAILED
            self.tc_result['error_info'] = 'VNF did not scale out'
            return False
        ...
        # --------------------------------------------------------------------------------------------------------------
        # 11. Trigger a resize of the VNF resources to use less specialized hardware by instructing the VNF to instruct
        #     the MANO to scale in the VNF
        # --------------------------------------------------------------------------------------------------------------
        LOG.info('Trigger a resize of the VNF resources to use less specialized hardware by instructing the VNF to '
                 'instruct the MANO to scale in the VNF')
        ...
        if self.vnf.scale_sync(self.vnf_instance_id, scale_type='in', aspect_id=self.tc_input['scaling']['aspect'],
                ...
                != constants.OPERATION_SUCCESS:
            LOG.error('TC_VNFC_SCALE_OUT_005__VNF_MANUAL execution failed')
            LOG.debug('VNF could not instruct the MANO to scale in the VNF')
            self.tc_result['overall_status'] = constants.TEST_FAILED
            self.tc_result['error_info'] = 'VNF could not instruct the MANO to scale in the VNF'
        ...
        LOG.info('TC_VNFC_SCALE_OUT_005__VNF_MANUAL execution completed successfully')

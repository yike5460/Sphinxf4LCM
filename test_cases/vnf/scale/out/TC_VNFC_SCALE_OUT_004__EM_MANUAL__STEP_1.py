...
class TC_VNFC_SCALE_OUT_004__EM_MANUAL__STEP_1(TestCase):
    ...
    def run(self):
        ...
        while scale_out_level <= self.tc_input['max_scale_out_steps'] + 1:
            if self.em.vnf_scale_sync(
                                   self.vnf_instance_id, scale_type='out', aspect_id=self.tc_input['scaling']['aspect'],
                                   additional_param={'scaling_policy_name': self.tc_input['scaling']['policies'][0]}) \
                    != constants.OPERATION_SUCCESS:
                LOG.error('TC_VNFC_SCALE_OUT_004__EM_MANUAL__STEP_1 execution failed')
                LOG.debug('EM could not instruct the MANO to scale out the VNF to the next level')
                self.tc_result['overall_status'] = constants.TEST_FAILED
                self.tc_result['error_info'] = 'EM could not instruct the MANO to scale out the VNF to the next level'

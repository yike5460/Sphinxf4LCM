...
class TC_VNFC_SCALE_OUT_005__EM_MANUAL(TestCase):
    """
    TC_VNFC_SCALE_OUT_005__EM_MANUAL Removal of virtualized specialized hardware acceleration for VNFC scale-in with
    manual scaling event generated by EM
    ...
    5. Trigger a resize of the VNF resources to use more specialized hardware by instructing the EM to instruct the
    ...
    def setup(self):
        ...
        LOG.info('Finished setup for TC_VNFC_SCALE_OUT_005__EM_MANUAL')

        return True

    def run(self):
        LOG.info('Starting TC_VNFC_SCALE_OUT_005__EM_MANUAL')
        ...
        if self.vnf_instance_id is None:
            LOG.error('TC_VNFC_SCALE_OUT_005__EM_MANUAL execution failed')
        ...
        # --------------------------------------------------------------------------------------------------------------
        # 5. Trigger a resize of the VNF resources to use more specialized hardware by instructing the EM to instruct
        #    the MANO to scale out the VNF
        # --------------------------------------------------------------------------------------------------------------
        LOG.info('Triggering a resize of the VNF resources to use more specialized hardware by instructing the EM to '
        ...
        # --------------------------------------------------------------------------------------------------------------
        # 11. Trigger a resize of the VNF resources to use less specialized hardware by instructing the EM to instruct
        #     the MANO to scale in the VNF
        # --------------------------------------------------------------------------------------------------------------
        LOG.info('Triggering a resize of the VNF resources to use less specialized hardware by instructing the EM to '
                 'instruct the MANO to scale in the VNF')
        self.time_record.START('scale_in_vnf')
        if self.em.vnf_scale_sync(self.vnf_instance_id, scale_type='in', aspect_id=self.tc_input['scaling']['aspect'],
                                  additional_param={'scaling_policy_name': self.tc_input['scaling_policy_name']}) \
                != constants.OPERATION_SUCCESS:
            LOG.error('TC_VNFC_SCALE_OUT_005__EM_MANUAL execution failed')
            LOG.debug('EM could not instruct the MANO to scale in the VNF')
            self.tc_result['overall_status'] = constants.TEST_FAILED
            self.tc_result['error_info'] = 'EM could not instruct the MANO to scale in the VNF'
        ...
        LOG.info('TC_VNFC_SCALE_OUT_005__EM_MANUAL execution completed successfully')

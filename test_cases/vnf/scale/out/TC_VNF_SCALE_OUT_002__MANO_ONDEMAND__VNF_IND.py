...
class TC_VNF_SCALE_OUT_002__MANO_ONDEMAND__VNF_IND(TestCase):
    """
    TC_VNF_SCALE_OUT_002__MANO_ONDEMAND__VNF_IND Max scale-out VNF instance with on-demand scaling event generated by
    MANO. The stimulus for scaling out consists of value changes of one or multiple VNF related indicators on the
    interface produced by the VNF.
    ...
    6. Trigger a resize of the NS resources to the maximum by altering a VNF indicator
    ...
    def setup(self):
        LOG.info('Starting setup for TC_VNF_SCALE_OUT_002__MANO_ONDEMAND__VNF_IND')
        ...
        LOG.info('Finished setup for TC_VNF_SCALE_OUT_002__MANO_ONDEMAND__VNF_IND')

    def run(self):
        LOG.info('Starting TC_VNF_SCALE_OUT_002__MANO_ONDEMAND__VNF_IND')
        ...
        while elapsed_time < constants.SCALE_INTERVAL:
            ns_info = self.mano.ns_query(filter={'ns_instance_id': self.ns_instance_id})
            if len(ns_info.vnf_info_id) == self.tc_input['scaling']['max_instances']:
                break
            else:
                time.sleep(constants.POLL_INTERVAL)
                elapsed_time += constants.POLL_INTERVAL
            if elapsed_time == constants.SCALE_INTERVAL:
                LOG.error('TC_VNF_SCALE_OUT_002__MANO_ONDEMAND__VNF_IND execution failed')
                LOG.debug('NS did not scale out to the max')
                self.tc_result['overall_status'] = constants.TEST_FAILED
                self.tc_result['error_info'] = 'NS did not scale out to the max'
        ...
        LOG.info('TC_VNF_SCALE_OUT_002__MANO_ONDEMAND__VNF_IND execution completed successfully')

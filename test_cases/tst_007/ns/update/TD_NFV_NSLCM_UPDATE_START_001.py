...
class TD_NFV_NSLCM_UPDATE_START_001(TestCase):
    ...
    def run(self):
        ...
        LOG.info('Verifying that other existing compute resources have not been affected by the performed operation by '
                 'querying the VIM')
        ns_info_after_start = self.mano.ns_query(filter={'ns_instance_id': self.ns_instance_id,
                                                         'additional_param': self.tc_input['mano'].get('query_params')})
        for vnf_info in ns_info_after_start.vnf_info:
            if vnf_info.instantiated_vnf_info.vnf_state != constants.VNF_STARTED:
        ...
        resolved_traffic_config = self.mano.resolve_ns_cp_addr(ns_info_after_start,
        ...
        if not self.mano.validate_ns_released_vresources(ns_info_after_start):

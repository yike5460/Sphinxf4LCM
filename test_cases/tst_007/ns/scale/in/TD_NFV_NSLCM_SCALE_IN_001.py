...
class TD_NFV_NSLCM_SCALE_IN_001(TestCase):
    ...
    def step2(self):
        ...
        ns_info_after_instantiation = self.mano.ns_query(query_filter={'ns_instance_id': self.ns_instance_id,
        ...
        for vnf_info in ns_info_after_instantiation.vnf_info:
            self.tc_result['resources'][
                '%s / %s (After instantiation)' % (vnf_info.vnf_product_name, vnf_info.vnf_instance_id)] = {}
            self.tc_result['resources'][
    ...
    def step4(self):
        ...
        ns_info_before_scale_in = self.mano.ns_query(query_filter={'ns_instance_id': self.ns_instance_id,
                                                                   'additional_param': self.tc_input['mano'].get(
                                                                       'query_params')})

        for vnf_info in ns_info_before_scale_in.vnf_info:
            self.tc_result['resources'][
                '%s / %s (Before scale in)' % (vnf_info.vnf_product_name, vnf_info.vnf_instance_id)] = {}
            self.tc_result['resources'][
    ...
    def step6(self):
        ...
        for vnf_info in self.ns_info_after_scale_in.vnf_info:
            self.tc_result['resources'][
                '%s / %s (After scale in)' % (vnf_info.vnf_product_name, vnf_info.vnf_instance_id)] = {}
            self.tc_result['resources'][

...
class OpenbatonManoAdapter(object):
            ...
            for vdu in constituent_vnfr.get('vdu'):
                for vnfc_instance in vdu.get('vnfc_instance'):
                    ...
                    for ext_cp in vnfc_instance.get('vnfComponent').get('connection_point'):
                        ...
                        virtual_link_reference = ext_cp.get('virtual_link_reference')
                        address_list = list()
                        for ip in vnfc_instance['ips']:
                            if ip['netName'] == virtual_link_reference:
                                address_list.append(str(ip['ip']))
                                break
                        vnf_ext_cp_info.address = address_list
    ...
    def get_vnfd(self, vnfd_id):
        ...
        method = 'get'
        try:
            resp, vnfd = self.do_request(url=url, method=method)
            assert resp.status_code == 200
        except Exception as e:
            LOG.exception(e)
            raise OpenbatonManoAdapterError('Unable to retrieve config for VNFD with ID %s. Reason: %s' % (vnfd_id, e.message))
        return vnfd
    ...
    def validate_vnf_allocated_vresources(self, vnf_info):
        ...
        for vdu in vnfd.get('vdu'):
            ...
            if bool(vdu.get('computation_requirement')):
    ...
    def verify_vnf_nsd_mapping(self, ns_instance_id, additional_param=None):
        ...
        for vnfd in nsd.get('vnfd'):
            vnf_name = str(vnfd.get('name'))
            expected_vnf_vnfd_mapping[vnf_name] = str(vnfd.get('id'))

        ns_info = self.ns_query(filter={'ns_instance_id': ns_instance_id, 'additional_param': additional_param})
        for vnf_info in ns_info.vnf_info:
            vnf_name = vnf_info.vnf_product_name
            if vnf_info.vnfd_id != expected_vnf_vnfd_mapping[vnf_name]:
                return False
    ...
    def get_nsd(self, nsd_id):
        ...
        method = 'get'
        try:
            resp, nsd = self.do_request(url=url, method=method)
            assert resp.status_code == 200
        except Exception as e:
            LOG.exception(e)
            raise OpenbatonManoAdapterError('Unable to retrieve config for NSD with ID %s. Reason: %s' % (nsd_id, e.message))
        return nsd
    ...
    def get_vnf_mgmt_addr_list(self, vnf_instance_id, additional_param=None):
        ...
        try:
            ...
            LOG.exception(e)
            raise OpenbatonManoAdapterError('Unable to retrieve config for VNF with ID %s. Reason: %s' % (vnf_instance_id, e.message))
